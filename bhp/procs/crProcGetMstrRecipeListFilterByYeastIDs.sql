...
if object_id(N'[bhp].GetMstrRecipeListFilteredByYeast',N'P') is not null
begin
	Drop Proc [bhp].GetMstrRecipeListFilteredByYeast;
	Print 'Proc:: [bhp].GetMstrRecipeListFilteredByYeast dropped!!!';
end
go

Create Proc [bhp].GetMstrRecipeListFilteredByYeast (
	@SessID varchar(256),
	@CustID bigint = null,
	@YeastCSVList varchar(2000) = null -- list of hop id's to use in the search...must be a csv list!!!
)
with encryption
as
	Declare @rc int;
	...
	Set @YeastCSVList = Case When @YeastCSVList = Space(0) Then Null Else @YeastCSVList End;

	--Raiserror(N'proc:: GetMstrRecipeListFilteredByYeast -> @CustID:[%I64d] @YeastCSVList:[%s]...',0,1,@CustID, @YeastCSVList);
	...
	Begin
			...
			Select * Into #csv From string_split(@YeastCSVList, ',');
			Set @tot = @@ROWCOUNT;
			
			While Exists (Select 1 From #csv Where [value] > @val)
			Begin
				Select Top (1) @val = [value], @wclause = COALESCE(@wclause + ' OR ','') + N'RY.fk_YeastMstrID = ' + [value] 
				From #csv
				Where ([value] > @val)
				Order By [value];
			End
...
Inner Join (
	SELECT RJ.RowID
	FROM [bhp].[RecipeYeasts] RY
	inner join bhp.RecipeJrnlMstr RJ On (RY.fk_RecipeJrnlMstrID = RJ.RowID And RJ.RowID > 0)
	where (' + @wclause + ')
	group by RJ.RowID
...
exec [bhp].GetMstrRecipeListFilteredByYeast @SessID='00000000-0000-0000-0000-000000000000', @CustID=NULL, @YeastCSVList=''
exec [bhp].GetMstrRecipeListFilteredByYeast @SessID='00000000-0000-0000-0000-000000000000', @CustID=0, @YeastCSVList='24'; -- us-05
exec [bhp].GetMstrRecipeListFilteredByYeast @SessID='00000000-0000-0000-0000-000000000000', @CustID=1002, @YeastCSVList='24,18'; -- us05 & German Ale 1007

...
if object_id(N'[bhp].GenBurpHopSchedMstrMesg',N'P') is not null
begin
	drop proc [bhp].GenBurpHopSchedMstrMesg;
	print 'proc:: [bhp].GenBurpHopSchedMstrMesg dropped!!!';
end
go

create proc [bhp].GenBurpHopSchedMstrMesg (
...
begin
	...
	Select
		...
		@boilTime = ISNULL(TotBoilTime,60),
		@fk_BoilTimeUOMID = case U.RowID When 0 then [bhp].fn_GetUOMIdByNm('min') else U.RowID end,
		@BoilTimeUOM = case U.RowID When 0 then 'min' else U.UOM End
	From [bhp].HopSchedMstr A 
	Inner Join [di].vw_CustomerMstr C On (A.fk_CreatedBy = C.RowID)
	Inner Join [bhp].UOMTypes U On (A.fk_TotBoilTimeUOM = U.RowID)
	Where (A.RowID = @id);
	...
	set @mesg.modify('
		...
		insert (
			...
			<b:BoilTime>
				...
				<b:UOM id=''{sql:variable("@fk_BoilTimeUOMID")}''>{sql:variable("@BoilTimeUOM")}</b:UOM>
			</b:BoilTime>,
			<b:Creator custid=''{sql:variable("@custID")}'' uid=''{sql:variable("@custUID")}''>
				{sql:variable("@custNm")}
			</b:Creator>
		)
		into (/b:Burp_Belch/b:Payload/b:HopSched_Evnt/b:Info)[1]
	');
	-- stuff in any note/comment....
	set @mesg.modify('
		declare namespace b="http://burp.net/recipe/evnts";
		insert sql:variable("@notes") as last into (/b:Burp_Belch/b:Payload/b:HopSched_Evnt/b:Info)[1]
	');
	...
	set @mesg.modify('
		...
		) as last into (/b:Burp_Belch/b:Payload/b:HopSched_Evnt/b:Info)[1]
	');
	...
	begin
		...
		set @mesg.modify('
			...
			into (/b:Burp_Belch/b:Payload/b:HopSched_Evnt/b:Info/b:Sharing_Flags)[1]
		');
	end
	...
	*/
	...
	Begin
		...
		Select @mesg = @mesg.query('(/Burp_Belch/Payload/HopSched_Evnt/Info)');
	End
...
declare @m xml;
declare @rc int;
exec @rc = [bhp].GenBurpHopSchedMstrMesg @id=9, @evnttype='add', @InfoNodeOnly=0, @SessID=N'00000000-0000-0000-0000-000000000000', @mesg = @m output;

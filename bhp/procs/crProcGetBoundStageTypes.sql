...
If Object_ID(N'[bhp].GetBoundStages',N'P') IS NOT NULL
Begin
	Drop Proc [bhp].GetBoundStages;
	Print 'Proc:: [bhp].GetBoundStages Dropped!!!';
End
Go
...
*/
Create Proc [bhp].GetBoundStages (
	@SessID varchar(256),
...
Begin
	...
	Select @sql = ISNULL(@sql + char(10) + N'UNION ALL' + char(10),'') + N'select U.RowID, U.Name from [bhp].StageTypes U Inner Join [bhp].' + OBJECT_NAME(fkc.parent_object_id) + N' As X On (U.RowID = X.' + c.[Name] + N' And U.RowID > 0)'
	From sys.foreign_key_columns fkc
	Inner join sys.columns c on (fkc.parent_object_id = c.object_id and c.column_id = fkc.parent_column_id)
	Where fkc.referenced_object_id = OBJECT_ID(N'[bhp].StageTypes',N'U')
...
select * from #Foo
union all
select RowID, Name, 0 from [bhp].StageTypes Where (RowID Not In (Select RowID from #Foo) and RowID > 0)
Order By Name;
